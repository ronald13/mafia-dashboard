...
from prep import create_tornado, simple_bar, stacked_bar, simple_pie, draw_pie, make_spider, text_fig
...
color_schemes = ['rgb(243,243,243)', 'rgb(206,217,216)', 'rgb(152,171,184)', 'rgb(125,148,163)', 'rgb(93,114,125)',
              'rgb(67,84,92)']
# read data
teams = pd.read_json('data/2022/teams.json')
games = pd.read_json('data/2022/tstata.json').rename(columns={'id': 'game_id'})
referee = pd.read_csv('data/2022/referee.csv')
# Constants
games_number = 14
teams = teams.replace({'РБФ':'RBF', 'Mafia house 1': 'Mafia House I', 'Mafia house': 'Mafia House', 'Медуза 1': 'Медуза I'})

kchb2022 = Mafia(games, teams, referee, games_number)
def get_data(year):
    games_number = 14
    current_dir = f"data/{year}/"
    teams = pd.read_json(current_dir + 'teams.json')
    games = pd.read_json(current_dir + 'tstata.json').rename(columns={'id': 'game_id'})
    referee = pd.read_csv(current_dir + 'referee.csv')
    return games, teams, referee, games_number



kchb_by_years = {2022:Mafia(*get_data(2022)), 2021:Mafia(*get_data(2021)), 2020:Mafia(*get_data(2020)), 2019:Mafia(*get_data(2019)), 2018:Mafia(*get_data(2018))}
total_win = kchb2022.win_by_round()
#
tornado_df = pd.read_csv('data/2022/mafia_tornadov2.csv')
referee_voting = pd.read_csv('data/2022/referee_voting.csv')
smeta = pd.read_csv('data/2022/mafia-smeta.csv')
win_by_round = kchb2022.win_by_round()
players_table = kchb2022.get_table_distribution()
wind_by_round_referee = kchb2022.wind_by_round_referee()
...
def update_dropdown_values(round_win_multi_dropdown, round_win_referee_dropdown, table_setting_team_dropdown, tornado_team_dropdown_1, tornado_team_dropdown_2):
    ...
    return multi_team_list, teams_list_value, teams_list_value, teams_list_value, teams_list_value_2
...
def update_graph(year):
    ...
    team_winner = html.Div(
        [
                    html.Div([
                        html.Img(src=f'assets/place_{position}.png', style={'width': '60px', 'margin-bottom': '20px', }),
    ...
    personal_winner = html.Div(
        [
                    html.Div([
                        html.Img(src=f'assets/place_{position}.png', style={'width': '30px', 'margin': '20px 0 10px 0'}),
    ...
    add_awards =  html.Div(
        [
                                html.Div([
                                    html.Img(src=f'assets/{image_name_list[counter]}.png', style={'width': '30px', 'margin-right': '15px'}),
...
def update_graph(year):
    ...
    total_games = kchb.games.shape[0]
...
def update_graph(year, type_chart):
    ...
    referee_pie_layout = html.Div(
                                [
                                    dcc.Graph(figure=simple_pie(colors=['#E0DDAA', '#A27B5C'], title=referee_score['referee_name'].values[count], values=[el * 100 for el in referee_results['value'].values[count]]),
                                              config={'displayModeBar': False}, style={'font-family': 'Roboto', 'margin-bottom': '20px'}) for count in range(0, len(referee_results))
...
def update_graph(team, year):
    ...
    df_Active = kchb.get_table_distribution()
    df_Active = df_Active[df_Active['team_name'] == team]
    values = df_Active['table_number_count'].to_list()
    players = df_Active['player_name'].to_list()
    spider_layout = html.Div(
        [
            dcc.Graph(figure=make_spider(values[counter],
                      param=[players[counter], color_list[len(df_Active)][counter]]),
                      config={'displayModeBar': False},
                      style={'flex-basis':'25%'})
            for counter in range(0, PLAYERS_IN_TEAM)

        ], style={'display':'flex', 'width':'100%', 'flew-wrap':'wrap'})
...
def update_graph(team_list, year):
    ...
    if not kchb:
        return
    rounds = ['Тур ' + str(x) for x in range(1, games_number + 1)]
    ...
    total.update_xaxes(title='<b>Номер тура</b>')
    total.update_yaxes(title='<b>Победы</b>')
    return total
...
def update_graph(team, year):
    ...
    for i, ref in zip(range(0,len(referee)), referee):
        fig.add_traces(
            go.Bar(name=ref, x=rounds, y=pd.Series(df_Active['score'].values)[0][i], width=0.95,
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            # dcc.Graph(id='best-referee',  config={'displayModeBar': False}, style={'max-width': '100%', 'width': '100%', 'margin-bottom':'20px'}),
            # dcc.Graph(id='pie_referee', figure=pie_referee,  config={'displayModeBar': False}, style={'max-width': '100%', 'max-height': '100%'}),
            html.Div(id ='referee-statistic')],

            style=merge_two_dicts(rectangle, {'flex-direction': 'column', 'position':'relative'})),

        #     html.Div([
        #             html.Div("Смета турнира", style=title),
    ...
    html.Div([
        html.Div([
                html.P('Турнирная статистика за года', style=title),
                html.Div([
                            dbc.RadioItems(
                                    id="YearSelector",
        ...
        html.Div([
            html.P('Движение по играм', style=title),
            ...
            dcc.Graph(id='total_win',   config={'displayModeBar': False}, style={'max-width': '100%', 'width': '100%'}),
        ], style=merge_two_dicts(rectangle, {'flex-direction': 'column'})),

        html.Div([
            html.P('Количество побед по турам', style=title),
        ...
        html.Div([
            html.P('Рассадка турнира', style=title),
            ...
            html.Div(id='spider_layout', style={'display':'flex', 'width':'100%', 'flew-wrap':'wrap'})

        ], style=merge_two_dicts(rectangle, {'flex-direction': 'column'})),

        html.Div([
            html.P('Сравнительная характеристика ', style=title),
            ...
            html.Div([
                ...
                html.P('© 2022 ШIFFER Inc. Информация для забавы', style={'font-size':'15px', 'color': '#fff', 'margin':'0 10px'}),
                html.A("Перейти на сайт", href='https://statistics.shiffer.by/#/Turnir/41', target="_blank", style=link),
